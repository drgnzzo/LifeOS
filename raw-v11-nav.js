...
function _cerrarScrub(){
  /* E3-D21 — CONSOLIDAR, no reproducir: al cerrar, el asentado del
     imán ya pintó el estado final vía _pintarScrub. Aquí solo se
     fijan flags/nivel y, al aterrizar en 0, se encadena el DESPLIEGUE
     canónico del motor (tween 750ms — idéntico a su cierre de 1→0). */
  var p=_scrub.p, dir=_scrub.dir, ir=(p>=.5);
  _scrub.on=false;
  if(window._warpNivel) window._warpNivel(dir,0);
  var h=document.documentElement;
  if(dir>0){
    if(ir){ window.nivel=1; window.enTransicion=false; window.estado(); }
    else  { h.classList.remove('oculta-hud');
            window.enTransicion=false; window.estado(); }
  } else {
    if(ir){ window.nivel=0;
            window.tween(750,function(e2){window.gajos.spread=e2;},
              function(){ window.enTransicion=false;
                h.classList.remove('oculta-hud'); window.estado(); }); }
    else  { window.enTransicion=false; window.estado(); }
  }
}
/* rueda: fase de captura — sustituye al salto discreto del motor */
addEventListener('wheel',function(e){
  if(_formAbierto()){ e.stopImmediatePropagation(); return; }
  if(window.nivel===2){
